        ...
        for (g in mostPopularGenres) {
            if (mostPopularGenres[g] == genres[i].title) {
                div.innerHTML = div.innerHTML + '       '
                        + '<a href="#map" class="btn btn-dark btn-lg" id="' + genres[i].title + '" onclick="getFestivalsWithGenre(id);">'
...
function initialize() {
    ...
    google_map = new google.maps.Map(document
...
function getFestivalsWithDate(){
	...
	var json = new Array();
	json = getData("festivals?genre=&date="+date);
	if (json.length>0) {
...
function setupMap(json) {
    ...
    for (var i = 0; i < t.length; i++) {
        ...
        google.maps.event.addListener(pin, 'click', function(link) {
            index = i;
            ...
            div.innerHTML = "<h1 id='festName'>" + this.title + "</h1></br><h2 id='lineup'>Location: " + this.location + "</br></h2>";
            div.innerHTML = div.innerHTML + "<h2 id='lineup'>Starts at: " + this.begin
                    + "</br>Ends at: " + this.end + " </br></br>Lineup:</h2>";
            this.text.forEach(function(artist) {
                div.innerHTML = div.innerHTML + '<a href="#festInfo" class="btn btn-dark btn-lg" id="' + artist + '">'
                + artist + "</a>	";
            });         
